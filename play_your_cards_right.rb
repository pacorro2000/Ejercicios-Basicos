...
get '/'  do
	...
	values = %w{Ace 2 3 4 5 6 7 8 9 10 Jack Queen Knig}
	suits.each do |suit|
...
get '/:guess' do
	...
	if (params[:guess] == 'higher' and value < session[:value]) or (params[:guess] == 'lower' and value > session[:value])
		"Game Over! The card was the #{card}. You managed to make #{session[:guesses]} correct guess#{'s' unless session[:guesses] == 1} <a href='/'>Play Again</a>"
	else
		session[:guesses] += 1
		session[:value] = value
